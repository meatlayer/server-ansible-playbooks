...
$logDirectory = sprintf('/www/%s/logs/', $currentUser);
...
if (true === file_exists($settingsFile)) {
    ...
    $MgrVarnish = new MgrVarnish();
    $MgrVarnish->setEnabled($varnishEnabled);
    $MgrVarnish->setDeveloperMode(VARNISH_DEVELOPER_MODE);
    $MgrVarnish->setPurgeLogfile($varnishPurgeLogfile);
    $MgrVarnish->setServer($varnishServer);
    $MgrVarnish->setCacheTagPrefix($varnishCacheTagPrefix);
    $MgrVarnish->setCacheLifetime($varnishCacheLifetime);
    $MgrVarnish->setExcludes($varnishCacheExcludes);
    $MgrVarnish->setExcludedParams($varnishCacheExcludedParams);
    $headerRegisterCallback = function() use ($MgrVarnish) {
        $cacheTagPrefix = $MgrVarnish->getCacheTagPrefix();
        $MgrVarnish->sendCacheHeaders();
    };
    $registerShutdownCallback = function() use ($MgrVarnish) {
        $cacheTagPrefix = $MgrVarnish->getCacheTagPrefix();
        $MgrVarnish->shutdownPurge();
    };
